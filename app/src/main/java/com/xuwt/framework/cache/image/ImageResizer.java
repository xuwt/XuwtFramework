...
public class ImageResizer extends ImageWorker {
    ...
    /**
     * Initialize providing a single target image size (no width and height);
     *
     * @param context
     * @param imageSize
     */
    ...
    @Override
    protected Bitmap processBitmap(Object data, ProcessCallback callback) {
    	int resId = Integer.parseInt(String.valueOf(data));
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "processBitmap - " + resId);
        }
        return decodeSampledBitmapFromResource(mResources, resId, getImageCache());
    }
     ...
     *         that are equal to or greater than the requested width and height
     */
    public static Bitmap decodeSampledBitmapFromByte(byte[] bytes, ImageCache cache) {
    ...
    public static Bitmap decodeSampledBitmapFromDescriptor(
            FileDescriptor fileDescriptor, ImageCache cache) {
    ...
    public static int calculateInSampleSize(BitmapFactory.Options options) {
        ...
        int reqWidth = WindowManager.get().getScreenWidth();
        int reqHeight = reqWidth * height / width;
        int inSampleSize = 1;

        if (reqHeight > 0 && reqWidth > 0 && width > reqWidth) {
