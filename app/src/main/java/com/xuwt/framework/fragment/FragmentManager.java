 ...
 * to switch to the framework's implementation.  See the framework {@link com.xuwt.framework.fragment.FragmentManager}
 * documentation for a class overview.
 *
 * <p>Your activity must derive from {@link FragmentActivity} to use this. From such an activity,
 * you can acquire the {@link com.xuwt.framework.fragment.FragmentManager} by calling
...
public abstract class FragmentManager {
     ...
     * retrieved with {@link com.xuwt.framework.fragment.FragmentManager#getBackStackEntryAt(int)
    ...
    /**
     * Pop all state off the back stack.  Returns true if pop succ, 
     * else false.  This function is asynchronous -- it enqueues the
     * request to pop, but the action will not be performed until the application
     * returns to its event loop.
     */
    public abstract void popAllBackStack();
     ...
     * {@link #addOnBackStackChangedListener(com.xuwt.framework.fragment.FragmentManager.OnBackStackChangedListener)}.
     */
     ...
     * {@link #getFragment(android.os.Bundle, String)} will return the current
     ...
     * placed with {@link #putFragment(android.os.Bundle, String, Fragment)}.
     *
     ...
     * that is it can't use {@link #putFragment(android.os.Bundle, String, Fragment)} to
    ...
    protected boolean mHideFragmentInBackStack;
    protected int mHideFragmentInBackStackFromIndex;
    /**
     * hide all the fragments in back stacks
     */
    public void hideFragmentInBackStack(boolean value) {
    	mHideFragmentInBackStack = value;
    }
    /**
     * set hide all the fragments in back stacks from index
     */
    public void setHideFragmentInBackStackFromIndex(int value) {
    	mHideFragmentInBackStackFromIndex = value;
    }
}
...
final class FragmentManagerImpl extends FragmentManager {
	...
	public void popAllBackStack() {
		int backStackCount = getBackStackEntryCount();
    ...
    private void hidePrevFragment(Fragment fragment) {
        if(mHideFragmentInBackStack) {
        	Fragment f = null;
        	for(int i = mActive.size() - 1; i >= 0; i--) {
        		if(mActive.get(i) != null && mActive.get(i).equals(fragment)) {
        			if(i - 1 >= 0 && i - 1 >= mHideFragmentInBackStackFromIndex) {
        				f = mActive.get(i - 1);
        				break;
        			}
        		}
    ...
    private void showPrevFragment(Fragment fragment) {
        if(mHideFragmentInBackStack) {
        	Fragment f = null;
        	for(int i = mActive.size() - 1; i >= 0; i--) {
        		if(mActive.get(i) != null && mActive.get(i).equals(fragment)) {
    ...
    public void showFragmentImmediate(Fragment fragment) {
        if (DEBUG) Log.v(TAG, "show immediate: " + fragment);
        if (fragment.mHidden) {
            fragment.mHidden = false;
            if (fragment.mView != null) {
                fragment.mView.setVisibility(View.VISIBLE);
